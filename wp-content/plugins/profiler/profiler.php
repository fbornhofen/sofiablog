<?php
/*  Copyright 2007 - 2008 Eric Schulz.
    Copyright 2008 - 2013 Fabian Bornhofen.
    All Rights Reserved.
...
/*
  ...
  Author: Eric Schulz, Fabian Bornhofen
  Version: 1.2.8
...
add_filter('query_vars', 'pf_queryvars' );
function pf_queryvars( $qvars )
{
    $qvars[] = 'page';
    return $qvars;
}
...
function pf_get_vars($id)
{
    ...
    $vars['wp_capabilities'] = get_user_meta($id, 'wp_capabilities', true);
    // FIX 2012-07-21
    $vars['user_registered'] = $uinfo->user_registered;
...
function pf_get_enabledroles()
{
    ...
    $editorehemfrw = get_option('pf_show_roles_editorehemfrw');
    ...
    $roleslist = array('\'a:1:{s:13:\"administrator\";s:1:\"1\";}\''=>$administrator, 
                       ...
                       //'\'a:2:{s:4:"read";b:1;s:7:"level_0";b:1;}}s:13:"editorehemfrw";a:2:{s:4:"name";s:13:"EditorEhemFrw";\'' => $editorehemfr,
                       '\'a:1:{s:13:\"editorehemfrw\";s:1:"1";}\'' => $editorehemfrw,
                       '\'a:1:{s:13:\"editorehemfrw\";b:1;}\'' => $editorehemfrw, // compat
                       '\'a:1:{s:11:\"contributor\";s:1:"1";}\''=>$contributor, 
                       '\'a:1:{s:11:\"contributor\";b:1;}\''=>$contributor,  // compatibility with users added in wp2
                       '\'a:0:{}\''=>$none);
...
function pf_prepare_first_names()
{
    global $wpdb;

    $fnames = $wpdb->get_results("SELECT user_id, meta_value FROM $wpdb->usermeta WHERE meta_key='first_name'");
    $res = array();
    foreach ($fnames as $fname) {
        $res[$fname->user_id] = $fname->meta_value;
    }

    return $res;
}

function pf_prepare_last_names()
{
    global $wpdb;

    $lnames = $wpdb->get_results("SELECT user_id, meta_value FROM $wpdb->usermeta WHERE meta_key='last_name'");
    $res = array();
    foreach ($lnames as $lname) {
        $res[$lname->user_id] = $lname->meta_value;
    }

    return $res;
}

function pf_get_full_name($id, $fnames, $lnames)
{
    return $fnames[$id].' '.$lnames[$id];
}
...
function pf_recent_posts($id, $limit)
{
	...
	WHERE post_author = $id AND post_type = 'post' 
		AND post_status NOT LIKE 'pending'
		AND post_status NOT LIKE 'auto_draft'
...
function pf_output_single($id)
{
    ...
    $fnames = pf_prepare_first_names();
    $lnames = pf_prepare_last_names();	
    ...
    $output .= "<p>".pf_get_full_name($id, $fnames, $lnames)."</p>";
    if(get_option('pf_show_emails') == 'yes')
        $output .= "<p>E-Mail:  <a href=" . "mailto:" . $user_email . ">" . antispambot($user_email) . "</a></p>";
    if($user_url != '' && $user_url != 'http://')
        $output .= "<p>Website:  <a href=" . $user_url . " rel=\"nofollow\">$user_url</a></p>";
	
    $output .= "<p>Jahrgang:  " . pf_format_datetime(get_userdata($id)->user_registered) . "</p>";
    $output .= "<p>Einsatzland: ". get_cimyFieldValue($id, 'EINSATZLAND')."</p>";
    $output .= "<p>Projektstelle: ". get_cimyFieldValue($id, 'PROJEKT')."</p>";
	
    if($user_description != '')
    {
        $output .= "<h3>Informationen zu mir und meinem Dienst</h3>";
        $output .= "<p>".nl2br($user_description)."</p>";
    }
	
    //display an author's recent posts, if any
    if(get_option('pf_recent_posts_per_profile') > 0 && !empty($recent_posts))
        $output .= "<h3>Neueste Beitr&auml;ge</h3>";
    ...
    if(get_option('pf_recent_comments_per_profile') > 0 && !empty($recent_comments))
        $output .= "<h3>Neueste Kommentare</h3>";
...
function pf_insert_pagination_single()
{
    ...
    if($prev != $user)
        echo "<a href=", the_permalink(), $character, "user=", pf_get_display_name($prev), ">&laquo; Vorherige(r)</a>";
    else
        echo '&laquo; Vorherige(r)';
		
    //divider
    echo " - ";
		
    if($next != $user)
        echo "<a href=", the_permalink(), $character, "user=", pf_get_display_name($next), ">N&auml;chste(r) &raquo;</a>";
    else
        echo 'N&auml;chste(r) &raquo;';
...
function pf_prepare_directory()
{
    ...
    if($page < 1)
        $page = 1;
    echo "<div style=\"visibility: hidden\">page=$page</div>";
...
function pf_output_directory($start, $records, $lastpage)
{
    ...
    //	$output .= '<th>E-Mail</th>';
    $output .= '<th>Land</th>';
    $output .= '<th>Jahrgang</th>';
    $output .= '</tr>';

    $first_names = pf_prepare_first_names();
    $last_names = pf_prepare_last_names();	

    //foreach ($first_names as $id=>$f) {echo "$id, $f";}
    ...
    foreach($uids as $key=>$value)
    {
        ...
        $output .= "<td><a href=" . $_SERVER['REQUEST_URI'] . $character . "user=" . $vars['ID'] . ">" . pf_get_full_name($value, $first_names, $last_names) . "</a></td>";
        //if(get_option('pf_roles_enabled') == 'yes')
        //	$output .= "<td>" . $role . "</td>";
        //if(get_option('pf_show_emails') == 'yes')
        ...
        //else
        //	$output .= "<td />";
        $output .= "<td>".get_cimyFieldValue($value, 'EINSATZLAND')."</td>";
...
function pf_backlink($lastpage = '')
{
    ...
    if(isset($page) && $lastpage == '')
    {
        $output = 
            '<div class="pfback">' . 
            "<a href=" . get_permalink() . $character . "page=" . $page . ">&laquo; Zur&uuml;ck zur &Uuml;bersicht</a>" . 
            '</div>';
    }
    ...
    elseif($page > $lastpage)
    {
        ...
        $output = 
            '<div class="pfback">' . 
            "<a href=" . get_permalink() . $character . "page=" . $lastpage . ">&laquo; Zur&uuml;ck zur &Uuml;bersicht</a>" . 
            '</div>';
    }
    else
    {
        $output = 
            '<div class="pfback">' . 
            "<a href=" . get_permalink() . ">&laquo; Zur&uuml;ck zur &Uuml;bersicht</a>" . 
            '</div>';
    }
...
function pf_default_options()
{
    ...
    add_option('pf_recent_posts_per_profile', 15); //CHANGED
         ...
         Administrator
             ...
             Subscriber
                 ...
                 Author
                     ...
                     Editor
                         <br />
                         <label for="pf_show_roles_editorehemfrw"><input name="pf_show_roles_editorehemfrw" type="checkbox" id="pf_show_roles_editorehemfrw" value="yes" <?php checked('yes', get_option('pf_show_roles_editorehemfrw')); ?> />
                         Editor (ehem. Frw.)
                             <br />
                             </label>
